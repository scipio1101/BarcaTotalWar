        ...
        class HexGame {
            constructor(customConfig = null) {
                ...
                this.customConfig = customConfig; // 存储自定义配置
            ...
            initializeGame() {
                ...
                if (this.customConfig) {
                    console.log('[自定义战斗] 使用自定义配置初始化单位');
                    this.initializeCustomUnits(this.customConfig);
                } else {
                    console.log('[默认战斗] 使用默认配置初始化单位');
                ...
                if (this.currentPhase === 'deployment') {
                    if (this.customConfig) {
                        this.addGameLog(`📦 自定义战斗开始！已自动部署单位`);
                        this.addGameLog(`🏛️ 罗马军团已部署完毕`);
                        this.addGameLog(`🌊 迦太基军团已部署完毕`);
                        this.addGameLog(`💡 可拖拽单位调整部署，完成后点击"确认部署"按钮`);
                    } else {
            ...
            initializeCustomUnits(config) {
                // Load custom unit generation code
                // This method will be defined in a separate file due to size
                // For now, calling the external function
                if (typeof window.initializeCustomUnitsImpl === 'function') {
                    window.initializeCustomUnitsImpl.call(this, config);
                } else {
                    console.error('Custom units implementation not loaded!');
                }
            }
...
行动状态: 移动${unit.hasMoved ? '✅' : '⭕'} | 射击${unit.hasRangedAttacked ? '✅' : '⭕'} | 近战${unit.hasMeleeAttacked ? '✅' : '⭕'}`;
            ...
            updateMoveButtons() {
                ...
                if (this.currentPhase === 'movement') {
                    ...
                    // 近战阶段：只有在可以结束回合时才显示"结束回合"
                    // 其他时候（如刚撤退完成、正在进行近战）显示"下一阶段"
                    const canEndRound = this.meleeSubPhase === 'select_attacker' && 
                                       !this.meleeAttacker && 
                                       this.getAvailableSustainedMeleeUnits().length === 0;
                    nextPhaseBtn.textContent = canEndRound ? '结束回合' : '下一阶段';
                } else {
                    nextPhaseBtn.textContent = '下一阶段';
                }
            }
            ...
            executeChargeAttack() {
                try {
                    console.log(`[近战结算] 开始，自定义: ${!!this.customConfig}`);
                ...
                // 立即隐藏执行冲锋按钮（通过改变子阶段）
                this.meleeSubPhase = 'executing'; // 设置为执行中状态
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        // 冲锋/持续近战完成后，先检查是否还有可冲锋单位，再检查持续近战
                        setTimeout(() => {
                            if (wasChargeType) {
                                console.log('[近战系统] 冲锋近战完成，检查下一个可冲锋单位...');
                            } else {
                                console.log('[近战系统] 持续近战完成，检查下一个持续近战...');
                            }
                            
                            // 触发AI继续处理（AI会检查是否还有可冲锋单位）
                            if (window.battleAI && window.battleAI.enabled && window.battleAI.shouldControl()) {
                                console.log('[近战系统] 触发迦太基AI继续近战');
                                window.battleAI.takeTurn();
                            } else if (window.battleAIRome && window.battleAIRome.enabled && window.battleAIRome.shouldControl()) {
                                console.log('[近战系统] 触发罗马AI继续近战');
                ...
                return true;
                    ...
                    console.error('[近战结算错误]', error);
                    this.addGameLog(`❌ 近战结算时发生错误: ${error.message}`);
                    this.resetChargeState();
                    return false;
                }
            }
        ...
        finishDefenderSupport() {
            ...
            // 现在防守方选择完成，应该切换回攻击方
            if (!this.meleeAttacker) {
                console.error('[finishDefenderSupport] 错误：meleeAttacker为空');
                return;
            }
...
行动状态: 移动${unit.hasMoved ? '✅' : '⭕'} | 射击${unit.hasRangedAttacked ? '✅' : '⭕'} | 近战${unit.hasMeleeAttacked ? '✅' : '⭕'}`;
            ...
            nextPhase() {
                try {
                    console.log(`[阶段切换] 当前: ${this.currentPlayer}/${this.currentPhase}, 自定义: ${!!this.customConfig}`);
                    ...
                    if (this.currentPhase === 'movement') {
                        ...
                        this.rangedAttacker = null;
                        this.rangedTarget = null;
                        console.log(`[射击阶段] 玩家: ${this.currentPlayer}, 保留配置: ${!!this.customConfig}`);
                    ...
                    console.log(`[阶段切换完成] 新: ${this.currentPhase}, 配置: ${!!this.customConfig}`);
                } catch (error) {
                    console.error('[nextPhase错误]', error);
                    this.addGameLog(`❌ 切换阶段错误: ${error.message}`);
                }
            }

            endTurn() {
                try {
                    console.log(`[回合结束] 玩家: ${this.currentPlayer}, 配置: ${!!this.customConfig}`);
                    ...
                    console.log(`[回合结束完成] 新玩家: ${this.currentPlayer}, 配置保留: ${!!this.customConfig}`);
                } catch (error) {
                    console.error('[endTurn错误]', error);
                    this.addGameLog(`❌ 回合结束错误: ${error.message}`);
                }
            }
            ...
            resetGame() {
                ...
                // 恢复自定义配置
                this.customConfig = savedCustomConfig;
                
                this.initializeGame();
                
                if (savedCustomConfig) {
                    this.addGameLog(`📦 重新开始自定义战斗`);
                } else {
            ...
            saveGame() {
                try {
                    ...
                    const gameState = {
                        ...
                        customConfig: this.customConfig, // 保存自定义配置
            ...
            loadGame() {
                try {
                    ...
                    this.customConfig = gameState.customConfig || null; // 恢复自定义配置
            ...
            updateUI() {
                ...
                const infoDiv = document.getElementById('selected-unit-info');
                    ...
                    const action = this.targetPosition && this.targetPosition.action === 'move' ? '移动' : '攻击';
            ...
            checkGameEnd() {
                try {
                    ...
                    console.log(`[胜负判定] 罗马: ${romeCurrentPoints}/${this.initialPoints.rome} (${romePercentage.toFixed(1)}%), 自定义: ${!!this.customConfig}`);
            ...
            showVictoryModal(winner, romePoints, carthagePoints, romePercentage, carthagePercentage) {
                try {
                    console.log(`[胜利弹窗] 显示胜利弹窗，自定义: ${!!this.customConfig}`);
                    
                    // 🔗 如果是外部战斗模式，返回结果给主游戏
                    if (window.BattleBridge && window.BattleBridge.isExternalMode) {
                        console.log('🔗 外部战斗模式，返回结果');
                        window.BattleBridge.returnBattleResult(winner, romePoints, carthagePoints, romePercentage, carthagePercentage);
                    }
                    ...
                    console.error('[胜利弹窗错误]', error);
                    this.addGameLog(`❌ 显示胜利弹窗时发生错误: ${error.message}`);
                }
            }
