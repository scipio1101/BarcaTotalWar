...
class BattleAIController {
    ...
    enable() {
        ...
        setTimeout(() => {
            if (this.shouldControl()) {
                this.log('🎯 检测到当前是迦太基回合，立即启动AI', 'success');
                this.takeTurn();
            }
        }, 100);
    }
    ...
    setupDefenseRetreatListener() {
        ...
        const checkForDefenseRetreat = () => {
            if (this.enabled && !this.thinking &&
                ...
                this.game.currentPlayer === 'carthage' &&
                this.game.meleeTarget &&
                !this.processingRetreat) { // 防止重复处理

                console.log('[迦太基AI] 检测到防御方撤退阶段，立即处理');
    ...
    shouldControl() {
        ...
        if (this.game.currentPhase === 'deployment') {
            ...
            console.log(`[迦太基AI] shouldControl() deployment: ${should} (deploymentPhase=${this.game.deploymentPhase}, faction=${this.faction})`);
            return should;
        }

        // 其他阶段检查currentPlayer
        const should = this.game.currentPlayer === this.faction;
        console.log(`[迦太基AI] shouldControl() normal: ${should} (currentPlayer=${this.game.currentPlayer}, faction=${this.faction}, phase=${this.game.currentPhase})`);
        return should;
    }
     ...
     */
    async takeTurn() {
        console.log(`[迦太基AI] takeTurn() called, shouldControl: ${this.shouldControl()}, currentPlayer: ${this.game.currentPlayer}, phase: ${this.game.currentPhase}, subPhase: ${this.game.meleeSubPhase}`);
        if (!this.shouldControl()) {
            return;
        }
        ...
        try {
            ...
            if (this.game.currentPhase === 'melee' && this.game.meleeSubPhase === 'defender_choose_retreat' && this.game.currentPlayer === 'carthage') {
                console.log('[迦太基AI] 检测到防御方撤退阶段，直接处理');
    ...
    findRearAttackPosition(cavalryUnit, target, myUnits) {
        ...
        switch (targetDirection) {
            case 'north': rearAngles = [180, 150, -150]; break;
            case 'south': rearAngles = [0, 30, -30]; break;
            case 'east': rearAngles = [270, 240, -240]; break;
            case 'west': rearAngles = [90, 60, 120]; break;
        }
    ...
    async planInfantryMovement(unit, enemyUnits, myUnits) {
        ...
        // 如果有其他步兵，优先与它们保持阵型
        if (myInfantry.length > 0) {
            const formationPos = this.findInfantryFormationPositionWithSupport(unit, myUnits, enemyUnits);
            if (formationPos) {
                return formationPos;
            }
        }
        ...
        // 最后兜底：直接向敌人前进
        return this.calculateApproachPosition(unit, priorityTarget, 5);
    }
    ...
    async executeMoveToPosition(unit, targetPos) {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            if (hexElement) {
                ...
                while (waitCount < 15) {
                    ...
                    // 检查是否因为达到3步而自动完成（movePlan被清空）
                    if (this.game.moveState === 'none' && !this.game.movePlan && previousLength > 0) {
                        planUpdated = true;
                        successfulSteps++;
                    ...
                    if (this.game.moveState !== previousState && this.game.moveState !== 'planning') {
                        failureReason = `moveState变为${this.game.moveState}`;
                    }
        ...
        if (this.game.movePlan && this.game.movePlan.length > 0) {
            ...
            // 游戏已自动完成规划
            this.log(`  └─ ✓ 完成${successfulSteps}步规划（游戏自动保存）`, 'info');
        } else {
    ...
    generateStepCandidates(currentX, currentY, ndx, ndy, stepDist, unit) {
        ...
        for (const pos of positions) {
            ...
            // 额外检查：确保该位置没有任何单位的格子（避免点击到单位）
            let hasUnitAtPos = false;
            for (const u of this.game.units) {
                if (u.id === unit.id || !u.deployed) continue;
                
                // 获取单位占据的所有格子
                const size = this.game.getUnitSizeWithDirection ? 
                    this.game.getUnitSizeWithDirection(u) : 
                    { width: this.game.unitSizes[u.type] || 1, height: 1 };
                
                // 检查目标位置是否在这个单位的任何格子上
                for (let dx = 0; dx < size.width; dx++) {
                    for (let dy = 0; dy < size.height; dy++) {
                        if (u.x + dx === pos.x && u.y + dy === pos.y) {
                            hasUnitAtPos = true;
                            break;
                        }
                    }
                    if (hasUnitAtPos) break;
                }
                if (hasUnitAtPos) break;
            }
    ...
    async handleMeleePhase() {
        ...
        console.log(`[迦太基AI] handleMeleePhase() called, subPhase: ${this.game.meleeSubPhase}, currentPlayer: ${this.game.currentPlayer}`);
    ...
    async defenderChooseRetreat() {
        ...
        console.log(`[迦太基AI] defenderChooseRetreat() called, meleeTarget: ${this.game.meleeTarget?.name}, currentPlayer: ${this.game.currentPlayer}, subPhase: ${this.game.meleeSubPhase}`);
        ...
        // 确保当前玩家是迦太基且处于防御方撤退阶段
        if (this.game.currentPlayer !== 'carthage' || this.game.meleeSubPhase !== 'defender_choose_retreat') {
            this.log(`└─ 条件不满足: currentPlayer=${this.game.currentPlayer}, subPhase=${this.game.meleeSubPhase}`, 'warning');
            this.processingRetreat = false; // 重置标志
            console.log(`[迦太基AI] 等待条件满足后重试...`);
            // 等待一小段时间后重试
            await this.delay(500);
            if (this.game.currentPlayer === 'carthage' && this.game.meleeSubPhase === 'defender_choose_retreat' && !this.processingRetreat) {
                console.log(`[迦太基AI] 条件满足，重试撤退选择`);
        ...
        if (shouldRetreat) {
            ...
            setTimeout(() => {
                ...
                if (window.battleAIRome && window.battleAIRome.enabled && 
                    window.battleAIRome.shouldControl() && 
                    this.game.meleeSubPhase === 'select_attacker') {
                    console.log('[迦太基AI] 撤退完成，触发罗马AI继续选择下一个攻击者');
            ...
            setTimeout(() => {
                if (this.game.meleeSubPhase === 'select_attacker_support') {
                    ...
                    // 触发攻击方AI选择支援
                    if (window.battleAIRome && window.battleAIRome.enabled && window.battleAIRome.shouldControl()) {
                        window.battleAIRome.takeTurn();
                    }
                }
            }, 500);
    ...
    findBestFlankingPosition(cavalryUnit, target, myUnits) {
        ...
        switch (targetDirection) {
            case 'north': preferredAngles = [180, 135, -135, 90, -90]; break;
            case 'south': preferredAngles = [0, 45, -45, 90, -90]; break;
            case 'east': preferredAngles = [270, 225, -225, 180, 0]; break;
            case 'west': preferredAngles = [90, 45, 135, 0, 180]; break;
        }
        ...
        if (direction === 'horizontal') {
            for (let offsetX = -6; offsetX <= 6; offsetX++) {
                ...
                positions.push({ 
                    ...
                    y: anchorUnit.y,
                    priority: Math.abs(offsetX) <= 3 ? 2 : 1
                });
                positions.push({ 
                    x: anchorUnit.x + offsetX * 2, 
                    y: anchorUnit.y + 2,
...
// 导出供全局使用
window.BattleAIController = BattleAIController;

console.log('🤖 战棋AI控制器模块已加载');
