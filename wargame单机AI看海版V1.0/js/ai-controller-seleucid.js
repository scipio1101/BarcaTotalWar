...
const SeleucidAIController = {
    ...
    config: {
        ...
        controlledFaction: 'seleucid',  // AI控制的阵营
        ...
        secondaryTarget: 'byzantium' // 第二目标城市（动态调整）
    },
    ...
    strategies: {
        ...
        alexander: {
            ...
            // 双首要目标：同时进攻罗马和迦太基
            offensiveTargets: [
                { cityId: 'pella', priority: 900, description: '攻陷佩拉城' },
                { cityId: 'byzantium', priority: 900, description: '攻陷拜占庭' },
                { cityId: 'syracuse', priority: 850, description: '占领叙拉古' },
                { cityId: 'carthage', priority: 800, description: '占领迦太基' },
                { cityId: 'rome', priority: 750, description: '占领罗马' },
                { cityId: 'newcarthage', priority: 700, description: '占领新迦太基' }
            ],
            expansionPriority: {
        ...
        syracuse: {
            ...
            secondaryDefense: [
                { cityId: 'jerusalem', priority: 850, description: '守卫耶路撒冷' },
                { cityId: 'damascus', priority: 800, description: '守卫大马士革' },
        ...
        antiochus: {
            ...
            secondaryDefense: [
                { cityId: 'jerusalem', priority: 950, description: '死守耶路撒冷' },
                { cityId: 'damascus', priority: 900, description: '死守大马士革' },
                { cityId: 'bereusia', priority: 850, description: '守卫别勒乌西亚' },
            ...
            greeceRegion: {
                cityIds: ['antioch', 'jerusalem', 'damascus', 'bereusia', 'demetrias', 'philippopolis'],
                priority: 950,
                description: '巩固小亚细亚本土（核心）'
            },
            economicStrategy: {
    ...
    enable(strategy = 'alexander') {
        ...
        if (strategyConfig) {
            ...
            // addLog(`🤖 塞琉古AI已启用 - ${strategyConfig.name}`, 'system');
            // addLog(`   📜 战略说明：${strategyConfig.description}`, 'seleucid');
        } else {
            // addLog(`🤖 塞琉古AI已启用 - 默认战略`, 'system');
        }
        
        this.initializeCityTracking('seleucid');
        this.updateUI();
    },
    ...
    switchStrategy(strategy) {
        if (this.strategies[strategy]) {
            ...
            this.config.aggressiveness = strategyConfig.aggressiveness;
            this.config.economicFocus = strategyConfig.economicFocus;
            addLog(`⚔️ 塞琉古战略切换为：${strategyConfig.name}`, 'seleucid');
            addLog(`   📜 ${strategyConfig.description}`, 'seleucid');
        } else {
    ...
    recordLostCity(city) {
        ...
        // addLog(`💔 塞琉古失去了${city.name}（转为${lostToName}），重要度${importance}`, 'seleucid');
    ...
    calculateRecaptureWeights(lostCityId) {
        const factionArmies = armies.seleucid || [];
    ...
    getPriorityRecaptureTarget(army) {
        ...
        Object.keys(armyWeights).forEach(cityId => {
            // 检查城市是否仍然失守
            const city = cities.find(c => c.id === cityId);
            if (!city || city.faction === 'seleucid') return;
    ...
    evaluateAlliance() {
        ...
        let secondaryTarget = 'byzantium'; // 默认第二目标
        ...
        if (this.config.alliance !== newAlliance) {
            ...
            addLog(`\n🤝 ===== 塞琉古联盟状态变更 =====`, 'seleucid');
            addLog(`📊 ${allianceReason}`, 'seleucid');
            ...
            if (oldAlliance !== 'neutral') {
                ...
                addLog(`❌ 解除与${oldAllyName}的联盟`, 'seleucid');
            }
            
            if (newAlliance !== 'neutral') {
                const factionNames = {'rome': '罗马', 'carthage': '迦太基', 'macedonia': '马其顿', 'seleucid': '塞琉古', 'ptolemy': '托勒密'};
                const newAllyName = factionNames[newAlliance] || '未知';
                addLog(`✅ 与${newAllyName}结成联盟`, 'seleucid');
                // addLog(`🎯 第一目标：${this.getCityName(primaryTarget)}，第二目标：${this.getCityName(secondaryTarget)}`, 'seleucid');
            } else {
                addLog(`⚔️ 保持独立，多方互为敌对`, 'seleucid');
                // addLog(`🎯 第一目标：${this.getCityName(primaryTarget)}（东扩），第二目标：${this.getCityName(secondaryTarget)}`, 'seleucid');
            }
    ...
    updateAllianceStrategy() {
        ...
        if (alliance === 'carthage') {
            ...
            // addLog(`🎯 战略目标：第一目标-罗马，第二目标-叙拉古`, 'seleucid');
        }
            ...
            // addLog(`🎯 战略目标：第一目标-迦太基城，第二目标-叙拉古`, 'seleucid');
        }
        else {
            // 保持中立 → 三方混战
            dynamicStrategy.name = '三方混战';
            dynamicStrategy.description = '保持独立，罗马、迦太基、塞琉古三方互为敌对';
            dynamicStrategy.offensiveTargets = [
                { cityId: 'pella', priority: 950, description: '占领佩拉（首要目标）' },
                { cityId: 'byzantium', priority: 900, description: '占领拜占庭（第二目标）' },
                { cityId: 'syracuse', priority: 800, description: '占领叙拉古' }
            ];
            // addLog(`🎯 战略目标：第一目标-佩拉，第二目标-拜占庭`, 'seleucid');
        }
     ...
     */
    isAlly(faction) {
        if (faction === 'seleucid') return true;
     ...
     */
    isEnemy(faction) {
        if (faction === 'seleucid') return false;
    ...
    isFriendlyCity(city) {
        ...
        if (city.faction === 'seleucid') return true;
    ...
    evaluateSituation() {
        ...
        return {
            ...
            myArmies: armies.seleucid || [],
            myFunds: gameState.seleucidFunds,
            myDebt: gameState.seleucidDebt,
     ...
     */
    async executeTurn() {
        console.log('🔍 塞琉古AI executeTurn 开始');
        
        if (!this.shouldControl()) {
            console.log('🔍 塞琉古AI shouldControl 返回 false，退出');
            return;
        }

        // 检查全局暂停状态
        if (typeof gameState !== 'undefined' && gameState.paused) {
            console.log('⏸️ 塞琉古AI执行被暂停');
            return;
        }
        ...
        console.log('🔍 塞琉古AI 策略:', strategyName);

        // 检查城市变化
        this.checkCityChanges('seleucid');
        ...
        // 获取所有塞琉古军队
        const seleucidArmies = armies.seleucid || [];
        
        console.log('🔍 塞琉古军队数量:', seleucidArmies.length);
        
        if (seleucidArmies.length === 0) {
            addLog('⚠️ 没有可用的塞琉古军队', 'seleucid');
            console.log('🔍 塞琉古AI executeTurn 结束（无军队）');
            return;
        }
        ...
        // 为每支军队制定行动计划
        for (let i = 0; i < seleucidArmies.length; i++) {
            const army = seleucidArmies[i];
            console.log(`🔍 处理第 ${i+1}/${seleucidArmies.length} 支军队: ${army.commander}`);
        ...
        // addLog(`🤖 塞琉古AI回合结束`, 'seleucid');
        
        console.log('🔍 塞琉古AI executeTurn 完成');
    },
    ...
    async executeFactionActions() {
        ...
        if (situation.myFunds < 100 && situation.myDebt < 6000 && !this.borrowedThisTurn) {
            addLog(`💰 塞琉古资金紧张(${situation.myFunds})，债务${situation.myDebt}，执行借款`, 'seleucid');
            try {
                executeBorrow();
                this.borrowedThisTurn = true;
                await this.delay(1000);
            } catch (error) {
                addLog(`❌ 借款失败：${error}`, 'seleucid');
            }
        }
        ...
        if (raiseArmyDecision) {
            addLog(`🏛️ ${raiseArmyDecision.reason}`, 'seleucid');
                    try {
                        executeRaiseArmy();
                        await this.delay(1000);
                    } catch (error) {
                        addLog(`❌ 组军失败：${error}`, 'seleucid');
                    }
                }
    },
    ...
    shouldRaiseNewArmy(situation) {
        ...
        // 7.2 检查安条克是否有驻军
        const armiesAtantioch = situation.myArmies.filter(a => a.location === capitalCity);
        if (armiesAtantioch.length > 0) {
            return null; // 安条克有驻军，无法组军
        }
        
        // 7.3 检查是否有紧急威胁（安条克3步内有敌军）
        const threatsToantioch = situation.enemyArmies.filter(e => {
            const distance = this.calculateDistance(e.location, capitalCity);
            return distance <= 3;
        });
        
        if (threatsToantioch.length > 0) {
    ...
    async executeArmyTurn(army) {
        ...
        addLog(`⚔️ 【${army.commander}】开始行动`, 'seleucid');
        
        const combatPower = calculateCombatPower(army);
        addLog(`   战力：${combatPower} | 位置：${this.getCityName(army.location)}`, 'seleucid');
        ...
        if (decision) {
            ...
            addLog(`   ⏸️ ${army.commander} 本回合待命`, 'seleucid');
        }
        ...
        if (connectedCities && connectedCities.length > 0) {
            ...
            if (bestTarget) {
                ...
                return {
                    ...
                    target: bestTarget,
                    priority: 10,
                    reason: `无更优选项，移动到${targetCity.faction === 'seleucid' ? '己方' : targetCity.faction === 'neutral' ? '中立' : '敌方'}城市${this.getCityName(bestTarget)}`
                };
            }
        }
    ...
    evaluateEnemyInSameCity(army, situation) {
        ...
        if (primaryTarget) {
            ...
            if (targetCity && targetCity.faction !== 'seleucid') {
        ...
        // 计算实力对比（使用综合战力评估）
        const myResult = this.calculateComprehensivePower(army, army.location, 'seleucid');
    ...
    isPlanStillRelevant(army, plan, situation) {
        ...
        if (plan.type === 'move') {
            ...
            // 如果目标已经被占领，取消计划
            if (targetCity.faction === 'seleucid') {
                return false;
            }
            ...
            if (enemiesAtTarget.length > 0) {
                const myResult = this.calculateComprehensivePower(army, targetCityId, 'seleucid');
                const myPower = myResult.totalPower;
    ...
    evaluateAttackOptions(army, situation) {
        const currentCity = cities.find(c => c.id === army.location);
        if (currentCity && currentCity.isUnderSiege && currentCity.besiegingFaction === 'seleucid') {
            return null;
        }
    ...
    evaluateMoveOptions(army, situation) {
        ...
        for (const cityId of connectedCities) {
            ...
            // 进攻敌方城市
            if (city.faction !== 'seleucid' && city.faction !== 'neutral') {
    ...
    evaluateSiegeOptions(army, situation) {
        ...
        // 特殊判断：当安条克城失陷，且本方军队士气为5时，最高优先级围城
        if (currentCity.id === 'antioch' && 
            currentCity.faction !== 'seleucid' && 
            army.morale >= 5) {
            return {
                type: 'siege',
        ...
        if (!isTargetCity && !isLongStayCase) {
            if (currentCity.faction === 'seleucid' || currentCity.faction === 'neutral') {
                return null;
            }
        }
    ...
    evaluateFortifyOptions(army, situation) {
        ...
        if (currentCity.faction !== 'seleucid') {
            return null;
        }
    ...
    async makeArmyDecision(army) {
        ...
        // 注意：借款已移至executeFactionActions()阵营层面处理
        
        // 1. 首都防御（超高优先级800-1200）
        const defendantiochDecision = this.needDefendantioch(army, situation);
        if (defendantiochDecision) {
            decisions.push(defendantiochDecision);
        }
    ...
    /**
     * 检查是否需要防御安条克（参考罗马AI的needDefendRome）
     * 简化版：检测安条克周围的威胁
     */
    needDefendantioch(army, situation) {
        ...
        const antiochCity = cities.find(c => c.id === 'antioch');
        
        // 如果安条克不存在或已失陷
        if (!antiochCity) return null;
        
        // 特殊情况1：安条克被围城
        if (antiochCity.isUnderSiege && antiochCity.besiegingFaction !== 'seleucid') {
            if (army.location === 'antioch') {
                // 在安条克城内，必须突围
                const enemiesAtantioch = situation.enemyArmies.filter(e => e.location === 'antioch');
                
                if (enemiesAtantioch.length > 0) {
                    enemiesAtantioch.sort((a, b) => calculateCombatPower(a) - calculateCombatPower(b));
                    const targetEnemy = enemiesAtantioch[0];
                    
                    return {
                        type: 'attack',
                        target: targetEnemy,
                        priority: 1200,  // 超高优先级
                        reason: `🚨【安条克围城突围】无条件攻击围城敌军${targetEnemy.commander}`
                    };
                }
            }
        }
        ...
        // 特殊情况2：安条克已失陷
        if (antiochCity.faction !== 'seleucid') {
            // 所有军队优先收复安条克
            const pathToantioch = this.findPathToCity(army.location, 'antioch');
            if (pathToantioch && pathToantioch.length > 0) {
                return {
                    type: 'move',
                    target: pathToantioch[0],
                    priority: 1100,  // 极高优先级
                    reason: `🚨【收复安条克】首都失陷，立即收复`
                };
            }
        }
        ...
        const myDistance = this.calculateDistance(army.location, 'antioch');
        ...
        // 寻找威胁安条克的敌军
        const connectedToantioch = getConnectedCities('antioch');
        let nearestThreat = null;
        let minDistance = Infinity;
        
        for (const enemy of situation.enemyArmies) {
            const enemyDistance = this.calculateDistance(enemy.location, 'antioch');
        ...
        if (nearestThreat) {
            ...
            // 如果敌军就在相邻位置
            if (connectedToantioch.includes(enemyLocation)) {
                // 如果我方军队在安条克或相邻城市
                ...
                if (myConnected.includes(enemyLocation)) {
                    ...
                    if (enemyCheck.canMove) {
                        return {
                            ...
                            priority: 900,  // 很高优先级
                            reason: `【防御安条克】拦截${nearestThreat.commander}(距首都${minDistance}步)`
                        };
                    } else if (enemyCheck.shouldReinforce) {
                        // 需要增强
                        if (army.morale < 5) {
                            return {
                                type: 'reorganize',
                                priority: 850,
                                reason: `【防御安条克】整编后拦截${nearestThreat.commander}`
                            };
                        }
                    }
                }
            ...
            if (army.location !== 'antioch') {
                ...
                if (pathToantioch && pathToantioch.length > 0) {
                    return {
                        ...
                        target: pathToantioch[0],
                        priority: 800,
                        reason: `【防御安条克】发现威胁${nearestThreat.commander}，回防首都`
                    };
                }
            }
        }
    ...
    checkCapitalDefense(army, strategyConfig) {
        ...
        if (!capital || capital.faction !== 'seleucid') {
            return null;
        }
    ...
    decideAgainstEnemies(army, enemies) {
        ...
        addLog(`   ⚔️ 发现敌军！我方战力：${myPower}，敌方战力：${totalEnemyPower}`, 'seleucid');
    ...
    findRecaptureTarget(army) {
        const lostCitiesRecord = this.lostCities['seleucid'] || {};
    ...
    isFriendlyArmy(army) {
        ...
        // 塞琉古军队永远是友方
        if (army.faction === 'seleucid') return true;
    ...
    isEnemyArmy(army) {
        ...
        // 塞琉古军队永远不是敌方
        if (army.faction === 'seleucid') return false;
    ...
    checkEnemyAtTarget(army, targetCityId) {
        ...
        if (!enemyFaction || enemyFaction === 'seleucid' || this.isAlly(enemyFaction)) {
    ...
    /**
     * 第一目标战略决策（使用动态目标）
     * 为距离第一目标最近的2支塞琉古军队制定专门策略
     */
    evaluateFirstTargetStrategy(army, situation) {
        // 使用动态确定的第一目标
        const targetCityId = this.config.primaryTarget || 'pella';
        const targetCity = cities.find(c => c.id === targetCityId);
        
        if (!targetCity) return null;
        
        // 如果目标已经是塞琉古控制，不需要特殊策略
        if (targetCity.faction === 'seleucid') return null;
        ...
        const basePriority = 350; // 第一目标战略基础优先级（高于一般移动，低于保卫安条克）
        ...
        if (currentCity.faction === 'neutral') {
            if (stayTurns > 6) {
                ...
                const attitude = currentCity.seleucidAttitude || 0;
                return {
                    type: 'diplomacy',
                    target: currentCity,
    ...
    /**
     * 第二目标战略决策（使用动态目标）
     * 为距离第二目标最近的塞琉古军队制定专门策略（排除执行第一目标战略的军队）
     */
    evaluateSecondTargetStrategy(army, situation) {
        // 使用动态确定的第二目标
        const targetCityId = this.config.secondaryTarget || 'byzantium';
        const targetCity = cities.find(c => c.id === targetCityId);
            ...
            if (myPower > enemyPower * 1) {
                ...
                if (currentCity.faction === 'seleucid') {
                    ...
                    if (retreatTarget) {
                        ...
                        const factionDesc = retreatCity.faction === 'seleucid' ? '己方' : 
    ...
    async executeDecision(army, decision) {
        ...
        addLog(`   📋 决策：${decision.reason}`, 'seleucid');
            ...
            case 'borrow':
                ...
                console.log(`🔍 未知决策类型: ${decision.type}`);
                    addLog(`   ❌ 未知决策类型：${decision.type}`, 'seleucid');
                // 更新行动历史
                this.updateArmyActionHistory(army, false, false);
            }
    ...
    async executeMove(army, targetCityId) {
        const targetCity = cities.find(c => c.id === targetCityId);
        if (!targetCity) {
            addLog(`   ❌ 目标城市不存在：${targetCityId}`, 'seleucid');
            return;
        }

        addLog(`   🚶 ${army.commander} 前往 ${targetCity.name}`, 'seleucid');
        ...
        try {
            ...
            if (enemies.length > 0) {
                addLog(`   ⚔️ 到达后发现敌军，准备战斗...`, 'seleucid');
                await this.executeAttack(army, enemies[0]);
            }
        } catch (error) {
            const currentCity = cities.find(c => c.id === army.location);
            const currentCityName = currentCity ? currentCity.name : army.location;
            addLog(`   ❌ 军队【${army.commander}】从${currentCityName}移动到${targetCity.name}失败：${error}`, 'seleucid');
        }
    },
    ...
    async executePersuade(army) {
        ...
        addLog(`   💬 ${army.commander} 在 ${currentCity.name} 进行游说`, 'seleucid');
        
        try {
            await executeDiplomacy();
            await this.delay(500);
        } catch (error) {
            addLog(`   ❌ 游说失败：${error}`, 'seleucid');
        }
    },
    ...
    async executeFortify(army) {
        ...
        addLog(`   🏰 ${army.commander} 在 ${currentCity.name} 修筑防御`, 'seleucid');
        
        try {
            await executeFortify();
            await this.delay(500);
        } catch (error) {
            addLog(`   ❌ 修筑失败：${error}`, 'seleucid');
        }
    },
    ...
    async executeSiege(army) {
        ...
        addLog(`   ⚔️ ${army.commander} 围攻 ${currentCity.name}`, 'seleucid');
        
        try {
            // 围城实际上是通过showArmyActionModal并选择"围城"实现的
            // 但在AI中，我们需要判断是否可以围城
            const canSiege = currentCity.faction !== 'seleucid' && currentCity.faction !== 'neutral';
            
            if (canSiege) {
                // 围城逻辑已在主游戏循环中处理，这里只需记录
                addLog(`   🔥 正在围攻敌城${currentCity.name}`, 'seleucid');
            } else {
                addLog(`   ⚠️ 无法围城：${currentCity.name}不是敌方城市`, 'seleucid');
            }
            
            await this.delay(500);
        } catch (error) {
            addLog(`   ❌ 围城失败：${error}`, 'seleucid');
        }
    },
    ...
    async executeReorganize(army) {
        addLog(`   🔧 ${army.commander} 进行整编`, 'seleucid');
        
        try {
            executeReorganize();
            await this.delay(800);
        } catch (error) {
            addLog(`   ❌ 整编失败：${error}`, 'seleucid');
        }
    },
    ...
    async executeRecruit(army) {
        addLog(`   📣 ${army.commander} 进行征召`, 'seleucid');
        
        try {
            executeRecruit();
            await this.delay(800);
        } catch (error) {
            addLog(`   ❌ 征召失败：${error}`, 'seleucid');
        }
    },
    ...
    async handleAttackDecision(attackerArmy, defenderArmy, city) {
        // 检查是否是AI控制的阵营
        if (!this.config.enabled || attackerArmy.faction !== 'seleucid') {
            return true; // 不是AI控制的军队，继续进攻
        }

        // addLog(`🤖 塞琉古AI正在准备进攻...`, 'seleucid');

        // 第一步：请求所有可能的援军（包括同盟）
        addLog(`📢 ${attackerArmy.commander} 向附近己方军队和同盟请求援军...`, 'seleucid');
        
        const supportRequested = await this.requestAllSupport(attackerArmy, defenderArmy, city);
        
        if (supportRequested) {
            addLog(`✅ 援军请求已发送，等待支援到达`, 'seleucid');
            await this.delay(1500);
        } else {
            addLog(`ℹ️ 附近无可用援军，${attackerArmy.commander} 将独自进攻`, 'seleucid');
        }

        // 第二步：使用综合战力评估（包含潜在援军）
        const myResult = this.calculateComprehensivePower(attackerArmy, city.id, 'seleucid');
        const myPower = myResult.totalPower;
        ...
        addLog(`⚖️ 综合实力对比：我方${myPower.toFixed(0)} vs 敌方${enemyPower.toFixed(0)} (${powerRatio.toFixed(2)}:1)`, 'seleucid');

        // 第三步：决定是否继续进攻
        if (myPower > enemyPower) {
            addLog(`⚔️ 综合战力占据优势，${attackerArmy.commander} 发起进攻！`, 'seleucid');
            return true; // 继续进攻
        } else {
            addLog(`⚠️ 即使考虑潜在援军后仍处于劣势(${powerRatio.toFixed(2)}:1)，但已经承诺进攻`, 'seleucid');
    ...
    async requestAllSupport(requestingArmy, enemyArmy, city) {
        // 获取所有己方军队
        const myArmies = armies.seleucid || [];
        const supportArmies = [];
        ...
        addLog(`📡 发现${supportArmies.length}支可能的援军：`, 'seleucid');
        for (const { army, distance } of supportArmies) {
            ...
            addLog(`   - ${army.commander} (${cityName}，距离${distanceText})`, 'seleucid');
        }
        ...
        for (const { army, distance } of supportArmies) {
            ...
            if (diceTotal <= 10) {
                ...
                addLog(`   ❌ ${army.commander} 支援失败 (2D6=${diceTotal} > 10)`, 'seleucid');
            }
        ...
        if (totalSupported > 0) {
            addLog(`✅ 成功获得${totalSupported}支援军！`, 'seleucid');
            // 更新地图显示
    ...
    executeAIReinforcement(reinforcingArmy, requestingArmy, dice1, dice2) {
        ...
        if (totalSupport === 0) {
            addLog(`   ⚠️ ${reinforcingArmy.commander} 兵力不足，无法提供支援`, 'seleucid');
            return 0;
        }
        ...
        // 记录日志
        addLog(`   ✅ ${reinforcingArmy.commander} 承诺提供援军支援 (2D6=${dice1}+${dice2}，支援战力${supportPercentage}%)`, 'seleucid');
        ...
        if (details.length > 0) {
            addLog(`      援军战力：${details.join(', ')}（参与战斗计算但不转移兵力）`, 'seleucid');
        }
    ...
    async handleDefenseDecision(defenderArmy, attackerArmy, city) {
        ...
        if (!this.config.enabled || defenderArmy.faction !== 'seleucid') {
            return null; // 不是AI控制的军队
        }

        // addLog(`🤖 塞琉古AI正在评估防御策略...`, 'seleucid');

        // 第一步：AI自动请求同城和周边所有地区的援军
        addLog(`📢 ${defenderArmy.commander} 向附近己方军队请求援军...`, 'seleucid');
        ...
        if (supportRequested) {
            ...
            addLog(`ℹ️ 附近无可用援军，${defenderArmy.commander} 将独自应战`, 'seleucid');
        }

        // 第二步：使用综合战力评估（包含请求完援军后的战力）
        const myResult = this.calculateComprehensivePower(defenderArmy, city.id, 'seleucid');
        const myPower = myResult.totalPower;
        ...
        if (isMyCity) {
            // ========== (二) 当前处于友方城市（己方或联盟）==========
            const cityType = city.faction === 'seleucid' ? '己方' : '联盟';
            addLog(`📍 当前位于${cityType}城市 ${city.name}`, 'seleucid');
            
            if (myPower > enemyPower * 0.9) {
                // (1) 战力 > 敌方×0.9：会战
                addLog(`⚔️ 综合战力优势（${powerRatio.toFixed(2)}:1），${defenderArmy.commander} 决定进行会战！`, 'seleucid');
                return {
                    action: 'battle',
                ...
                addLog(`🛡️ 战力处于中等劣势（${powerRatio.toFixed(2)}:1），${defenderArmy.commander} 决定守城！`, 'seleucid');
                addLog(`📝 后续回合将优先征召和整编增强实力`, 'seleucid');
                ...
                if (retreatTarget) {
                    ...
                    if (this.isFriendlyCity(targetCity)) {
                        factionDesc = targetCity.faction === 'seleucid' ? '己方' : '联盟';
                    } else if (targetCity.faction === 'neutral') {
                        factionDesc = '中立';
                    }
                    addLog(`🏃 找到${factionDesc}城市可撤退，${defenderArmy.commander} 决定撤退至 ${targetCity.name}`, 'seleucid');
                    return {
                        action: 'retreat',
                    ...
                    // 周边无合适撤退目标，守城
                    addLog(`🛡️ 周边无合适撤退目标，${defenderArmy.commander} 决定守城死战！`, 'seleucid');
            ...
            addLog(`📍 当前位于${cityType} ${city.name}`, 'seleucid');
            ...
            if (myPower > enemyPower * 0.9) {
                ...
                if (retreatTarget) {
                    const targetCity = cities.find(c => c.id === retreatTarget);
                    const factionDesc = targetCity.faction === 'seleucid' ? '己方' : '中立';
                    addLog(`🏃 找到${factionDesc}城市可撤退，${defenderArmy.commander} 决定撤退至 ${targetCity.name}`, 'seleucid');
                    return {
                        action: 'retreat',
                    ...
                    // 周边无合适撤退目标，会战
                    addLog(`⚔️ 周边无合适撤退目标，${defenderArmy.commander} 决定进行会战！`, 'seleucid');
                    return {
                        action: 'battle',
    ...
    findBestRetreatTarget(defenderArmy, attackerArmy) {
        ...
        for (const cityId of connectedCities) {
            ...
            if (this.isSeaRoute(currentLocation, cityId)) {
                addLog(`   ❌ ${city.name}(海路) - 海路不能作为撤退路线`, 'seleucid');
                continue;
            }

            // 【关键2】只考虑中立城市和己方城市，不考虑敌方城市
            if (city.faction !== 'neutral' && city.faction !== 'seleucid') {
                addLog(`   ❌ ${city.name}(敌方城市) - 不考虑撤退到敌方城市`, 'seleucid');
                continue;
            }
            ...
            addLog(`   ${city.name}: ${reason}, 优先级${priority}`, 'seleucid');
